...
// UpdateGame replaces a game's mutable fields by id within scope. Returns
// NotFound when no row matches. Stable fields (id, scope, created_at, type) are
// not changed here.
func (db *DB) UpdateGame(ctx context.Context, g *types.Game) (*types.Game, error) {
	...
	res, err := db.execContext(ctx,
		`UPDATE games SET campaign_id=?, name=?, seed_generator=?, reward_handler=?, validator=?,
		                  status=?, rules=?, handler_config=?, validator_config=?, ui=?, wallet_scope=?, milestones=?, updated_at=?
		   WHERE id=? AND tenant_id=? AND merchant_id=?`,
		g.CampaignID, g.Name, g.SeedGenerator, g.RewardHandler, g.Validator,
		string(g.Status), string(rules), string(hc), string(vc), string(ui), g.WalletScope, string(ms), g.UpdatedAt,
		g.ID, g.Scope.TenantID, g.Scope.MerchantID)
	if err != nil {
		return nil, gkerr.New(gkerr.ReasonInternal, "update game").Wrap(err)
	}
	n, err := res.RowsAffected()
	if err != nil {
		return nil, gkerr.New(gkerr.ReasonInternal, "update game rows").Wrap(err)
	}
	if n == 0 {
		return nil, gkerr.New(gkerr.ReasonNotFound, "game not found").WithMeta("game_id", g.ID)
	}
	return g, nil
}
